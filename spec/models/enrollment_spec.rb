...
RSpec.describe Enrollment, type: :model do
  context 'all required fields are present' do
    ...
    let!(:applicant_detail) { FactoryBot.create(:applicant_detail, user:) }
    ...
    it 'is valid' do
      expect(FactoryBot.create(:enrollment, user:, session_registration_ids:,
    ...
    it 'not valid without session_registration' do
      ...
      course_registration_ids = []
      expect do
        FactoryBot.create(:enrollment, user:, session_registration_ids:, course_registration_ids:)
      end
        .to raise_error(ActiveRecord::RecordInvalid,
                        'Validation failed: Select at least one session, Select at least one course')
    end
    ...
    it 'prevent creating second enrollment records for a user for the current camp' do
      enroll1 = FactoryBot.create(:enrollment, user:, session_registration_ids:,
                                               course_registration_ids:)
