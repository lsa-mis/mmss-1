ActiveAdmin.register SessionActivity, as: 'Session Selection' do
  ...
  filter :enrollment_id, as: :select, collection: proc {
    Enrollment.current_camp_year_applications.map { |enrol| [enrol.display_name.downcase, enrol.id]}.sort
  }
  filter :camp_occurrence_id, label: "Session", as: :select, collection: proc {
  ...
  form do |f|
    f.inputs do
      ...
      f.input :camp_occurrence_id, label: "Session", as: :select, collection: proc {
        CampOccurrence.active.no_any_session
      }
    end
    f.actions
